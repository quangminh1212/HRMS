...
def set_header_footer(ws, title: str | None = None, username: str | None = None, org: str | None = None) -> None:
    """Set a simple header/footer: centered title (+ org), left footer generated by HRMS (+username), right footer timestamp + page numbers."""
    try:
        ttitle = title or ws.title or "Report"
        if org:
            t = f"{org} - {ttitle}"
        else:
            t = ttitle
        ts = datetime.now().strftime('%d/%m/%Y %H:%M')
        # openpyxl header_footer API
        if hasattr(ws, 'header_footer'):
            ws.header_footer.center_header = t
            ws.header_footer.left_footer = f"Generated by HRMS{(' - ' + username) if username else ''}"
            ws.header_footer.right_footer = f"{ts} - &P/&N"
        else:
            # fallback to oddHeader/oddFooter if attribute exists (older openpyxl)
            if hasattr(ws, 'oddHeader'):
                try:
                    ws.oddHeader.center.text = t
                    ws.oddFooter.left.text = f"Generated by HRMS{(' - ' + username) if username else ''}"
                    ws.oddFooter.right.text = f"{ts} - &P/&N"
                except Exception:
                    pass
    except Exception:
        # Non-critical
...
def set_freeze(ws, row: int = 2, col: int = 1) -> None:
    try:
